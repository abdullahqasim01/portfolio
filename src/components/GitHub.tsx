...
export function GitHub() {
    ...
    useEffect(() => {
        fetch("https://api.github.com/users/abdullahqasim01")
            .then((response) => response.json())
            .then((data: UserProfile) => setUser(data));

        fetch("https://api.github.com/users/abdullahqasim01/repos?sort=pushed&direction=asc&per_page=3")
            .then((response) => response.json())
            .then((data) =>
                setRepos(
                    data.map((repo: any) => ({
